...
export default function LiveSession() {
  ...
  const [techAlert, setTechAlert] = useState<any>(null);
  const [activeSopId, setActiveSopId] = useState<string | null>(null);
  const activeSopIdRef = useRef<string | null>(null);

  useEffect(() => { activeSopIdRef.current = activeSopId; }, [activeSopId]);
  ...
  useEffect(() => {
    const captureInterval = setInterval(() => {
      if (!videoRef.current || videoRef.current.readyState < 2 || !captureCanvasRef.current) return;
      ...
      fetch(`http://${window.location.hostname}:3001/api/analyze-live`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sessionId: sessionId,
          frameData,
          activeSopId: activeSopIdRef.current,
          timestamp: new Date().toISOString()
        })
      }).catch(err => console.log('Capture error:', err));
    }, 10000); // Increased interval to 10s to stay within Gemini Free Tier quota (15 RPM)
  ...
  useEffect(() => {
      ...
      handleResize();
       ...
       socket.on('connect', () => {
         setConnectionStatus("Connected to Server"); socket.emit("join-session", sessionId);
         initCamera();
       });
       ...
       socket.on('signal', async (data: any) => {
        ...
        socket.on('ai-alert', (alert: any) => {
          if (['CRITICAL', 'HIGH'].includes(alert.severity)) {
            setTechAlert({ severity: alert.severity, finding: alert.finding });
            if (navigator.vibrate) navigator.vibrate(200);
            setTimeout(() => setTechAlert(null), 5000);
          }
        });

        socket.on('sop-activated', (data: any) => {
           setActiveSopId(data.sopId);
        });
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      {techAlert && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md">
          <div className={`rounded-xl p-4 shadow-2xl border flex items-start gap-3 backdrop-blur-md animate-in slide-in-from-top-4 ${
            techAlert.severity === 'CRITICAL' 
              ? 'bg-red-500/90 border-red-400 text-white shadow-red-500/50' 
              : 'bg-orange-500/90 border-orange-400 text-white shadow-orange-500/50'
          }`}>
            <span className="text-2xl">⚠️</span>
            <div>
              <div className="font-bold uppercase tracking-wider text-xs opacity-90 mb-0.5">
                {techAlert.severity} ALERT
              </div>
              <div className="font-medium">{techAlert.finding}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/70 backdrop-blur-sm rounded-full px-6 py-3 z-30">
        <div className="flex items-center justify-between gap-6">
          ...
          <button onClick={toggleCamera} className={`w-12 h-12 rounded-full flex items-center justify-center ${isCameraOff ? 'bg-red-500' : 'bg-white/10'}`}>
      ...
      <div className="absolute top-1/2 right-4 -translate-y-1/2 flex flex-col gap-2 z-50">
        ...
        <button onClick={() => socketRef.current?.emit('clear-annotations', { sessionId: sessionId })} className="px-3 py-2 bg-red-500 rounded text-xs text-white font-medium">Clear 3D</button>
      </div>
      ...
      {showEndConfirm && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6 max-w-sm w-full text-center">
            ...
            <p className="text-white/60 mb-6">Are you sure you want to end this live remote assistance session?</p>
            <div className="flex gap-3">
              <button onClick={() => setShowEndConfirm(false)} className="flex-1 py-3 bg-white/10 text-white rounded-xl font-medium hover:bg-white/20 transition-colors">
                Cancel
              </button>
              <button onClick={handleEndSession} className="flex-1 py-3 bg-[#E24B4A] text-white rounded-xl font-medium hover:opacity-90 transition-opacity">
                End Session
              </button>
            </div>
          </div>
